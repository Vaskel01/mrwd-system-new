    ...
    <section className="card rounded-xl p-5 no-print" aria-labelledby="task-actions-title">
      ...
      <div className="mt-4 space-y-3">
        {nextTaskStatus ? (
          ...
          <div className="rounded-lg border border-violet-200 bg-violet-50 px-4 py-3 text-sm font-bold text-violet-800">Field work is complete and waiting for WDLCD verification.</div>
  ...
  return <div className="space-y-5 complaint-receipt">
    ...
    {complaint.status === 'rejected' && <button onClick={() => setRestoreOpen(true)} className="btn-primary w-full rounded-lg">Undo rejection</button>}
        ...
        {canEcmdOperate && <section className="card rounded-xl p-5">
          <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-wider text-violet-600">ECMD · WDLCD</p><h2 className="mt-1 font-display font-bold text-navy-900">Dispatch and verification</h2><p className="mt-1 text-xs text-gray-500">Assign field work and verify completed work before resolving the complaint.</p></div><StatusBadge status={complaint.status} /></div>
          <div className="mt-4 space-y-2">
