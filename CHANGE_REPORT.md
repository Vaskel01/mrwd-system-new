...
- Refined the login water artwork with clearer independent wave motion, a restrained gold crest, and gentle pointer-following parallax. The layers now build into a curling wave at the white login-panel boundary to create a subtle crash-and-recede effect. The earlier sweeping shine and cursor glow were removed. Feature rows retain a small hover response, while reduced-motion users receive a static treatment.
...
- Login motion check: passed in the production preview; the crash artwork remains contained within the lower 35%, pointer movement updates only the parallax variables, no cursor glow or sweeping shine remains, and reduced-motion disables both animation and pointer response
