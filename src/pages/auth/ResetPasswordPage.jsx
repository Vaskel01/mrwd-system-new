...
export default function ResetPasswordPage() {
  ...
  const submit = async event => {
    ...
    if (!isPasswordValid(password)) return setError('Use at least 8 characters with at least one letter and one number.')
      ...
      <div className="w-full max-w-md card rounded-2xl overflow-hidden">
        ...
        <form onSubmit={submit} className="p-6 space-y-4">
          ...
          {done ? <div className="rounded-lg bg-green-50 border border-green-200 p-4 text-sm text-green-800 font-bold">Password updated. Redirecting to sign in…</div> : <>
            <div><label className="block text-sm font-semibold text-gray-700 mb-2">New password</label><input name="new_password" aria-label="New password" type="password" autoComplete="new-password" value={password} onChange={e => setPassword(e.target.value)} className="input-field rounded-lg" /><PasswordStrengthMeter password={password} /></div>
