...
## Department staff workflows

| ID | Test | Expected result |
|---|---|---|
| A-01 | Create Department Staff and Maintenance Personnel accounts | Accounts are created with the selected access only through System Administration > Staff Accounts. |
| A-02 | Deactivate staff with no active task | Login is blocked until the account is reactivated. |
| A-03 | Deactivate staff with an active task | The system refuses and asks the System Supervisor to have ECMD reassign active tasks first. |
...
| M-04 | Save ETA and materials | Customer and ECMD details show the saved plan and timeline entry. |
...
| M-07 | Request additional assistance | ECMD Staff are notified and the reason is visible without closing the assignment. |
| M-08 | Request reassignment | Task becomes Needs Attention; ECMD Staff can reassign it. |
| M-09 | Report cannot complete | Task becomes Needs Attention and the reason is logged. |
| M-10 | Set On Leave in My Profile | ECMD Staff see the availability and cannot select that Maintenance Personnel account for a new assignment. |
...
| UI-10 | Open Priority Score help and the Commercial classifier analysis | Thresholds are explained and the score composition diagram matches the numerical breakdown. |
...
| QOL-02 | Change the Complaint Reports date range and export CSV | Cards, complaint charts, and exported complaints use the selected filing dates. ECMD workload remains on Field Operations. |
...
| QOL-08 | Set Complaint Dispatch filters, open a record, and use browser Back | View, search, priority, status, personnel, sort, and page are restored from the URL. |
...
| OPS-07 | Request archival and attempt self-approval | Self-approval is rejected; a different authorized System Supervisor can approve and archive. |
...
| DEPT-07 | Sign in with an unassigned Department Staff account | The account is labeled Restricted Department Staff and cannot enter any department module until assigned by a System Supervisor. |
| DEPT-08 | Change a Department Staff account's access in Staff Accounts | The next authenticated profile refresh shows only the new department pages; the action is recorded in the audit trail. |
