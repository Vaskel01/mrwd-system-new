...
export default function SavedViewsBar({ moduleKey, filters, onApply }) {
  ...
  return <div className="flex flex-wrap items-center gap-2 rounded-xl border border-gray-200 bg-white p-2.5">
    ...
    <div className="flex min-w-[220px] flex-1 gap-2"><input value={name} onChange={e => setName(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); create() } }} placeholder="Name these filters" className="input-field min-h-9 rounded-lg py-1.5 text-xs"/><button type="button" onClick={create} disabled={saving || name.trim().length < 2} className="btn-secondary min-h-9 rounded-lg px-3 py-1.5 text-xs disabled:opacity-40">Save view</button></div>
