    ...
    <div className="space-y-5">
      ...
      <div className="page-band wave-header rounded-2xl overflow-hidden px-4 sm:px-6 py-5 sm:py-6 relative">
        ...
        <p className="text-navy-300 text-sm mt-1">
          {sorted.length} {sorted.length === 1 ? 'service advisory' : 'service advisories'} published · visible to customers
        </p>
      </div>
      ...
      {showForm && (
        <div className="bg-white border border-gray-200 mb-5 overflow-hidden">
          <div className="bg-gray-50 border-b border-gray-200 px-5 py-3">
            <p className="text-xs font-black text-gray-500 uppercase tracking-widest">{editing ? 'Edit advisory' : 'New advisory'}</p>
          </div>
          <form onSubmit={handleSubmit(onSubmit)} className="p-4 sm:p-5 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="sm:col-span-2">
                ...
                <input aria-label="Title" type="text" placeholder="Example: Scheduled water interruption – June 20"
                  {...register('title')}
            ...
            <div>
              <label className="block text-xs font-black text-gray-500 uppercase tracking-wider mb-1.5">Message <span className="text-red-500">*</span></label>
              <textarea aria-label="Content" rows={4} placeholder="Write the information customers need to know."
                {...register('content')}
            ...
            <label className="flex items-start gap-3 rounded-lg border border-gold-200 bg-gold-50 p-3 cursor-pointer">
              ...
              <span><span className="block text-sm font-bold text-navy-900">Pin as important</span><span className="block text-xs text-gray-500 mt-0.5">Important advisories appear above regular notices.</span></span>
            </label>
            <div>
              <label className="block text-xs font-black text-gray-500 uppercase tracking-wider mb-1.5">Show until <span className="normal-case font-medium text-gray-500">(optional)</span></label>
              <input aria-label="Show until" type="datetime-local" {...register('active_until')} className="input-field" />
              <p className="mt-1.5 text-xs text-gray-500">After this date, customers will no longer see the advisory. Commercial Services can still review it here.</p>
            </div>
            <div className="flex gap-3 pt-1">
              <button type="submit" disabled={posting}
                className="btn-primary flex items-center gap-2">
                {posting
                  ? <><div className="w-4 h-4 border-2 border-white border-t-transparent animate-spin"/>Publishing…</>
                  : <><AppIcon name="announcement" className="h-4 w-4" />{editing ? 'Save changes' : 'Publish advisory'}</>
                }
              </button>
            </div>
          </form>
                ...
                <div className="flex flex-col min-[520px]:flex-row items-start gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500">
                      ...
                      {a.active_until && <> · Shown until {new Date(a.active_until).toLocaleString('en-PH', { month: 'short', day: 'numeric', year: 'numeric', hour: 'numeric', minute: '2-digit' })}</>}
                    </p>
                  </div>
                  ...
                  <div className="flex w-full min-[520px]:w-auto shrink-0 flex-wrap items-center justify-end gap-1">
                    ...
                    <button onClick={() => handleImportance(a)}
                      title={a.is_important ? 'Remove important status' : 'Pin as important'}
                      ...
                      {a.is_important ? 'Remove pin' : 'Pin as important'}
                    </button>
