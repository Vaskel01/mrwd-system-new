...
export default function AdminDashboard() {
  ...
  const inProgress  = complaints.filter(c => ['en_route', 'in_progress'].includes(c.status)).length
    ...
    <div className="space-y-6">
      ...
      <div className="page-band wave-header rounded-2xl overflow-hidden px-6 py-7 relative">
        ...
        <div className="relative flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          <div>
            <p className="text-gold-400 text-[11px] font-bold uppercase tracking-[0.15em] mb-2">Administrator Command Center</p>
      ...
      {unassigned > 0 && (
        <button onClick={() => navigate('/admin/assign')}
          ...
          <div>
            ...
            <p className="text-xs text-amber-700 mt-0.5">Unassigned complaints delay resolution — assign Maintenance Personnel now</p>
          </div>
              ...
              {recent.map((c, i) => (
                <tr key={c.id} style={{ borderBottom: i < recent.length - 1 ? '1px solid #f8f9fb' : 'none' }}
                  ...
                  <td className="px-5 py-3.5">
                    <p className="font-semibold text-gray-900">{c.complaint_type}</p>
                    <p className="mt-0.5 text-[10px] font-mono font-bold text-gray-400">{c.reference_number}</p>
                  </td>
          ...
          {recent.map(c => (
            <div key={c.id} onClick={() => navigate(`/complaints/${c.id}`)} className="p-4 flex items-center gap-3 cursor-pointer hover:bg-gray-50">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] font-mono font-bold text-gray-400">{c.reference_number}</p>
