...
function categoryEvidenceLabel(confidence) {
  if (confidence == null || !Number.isFinite(Number(confidence))) return null
  if (Number(confidence) >= 75) return 'Strong'
  if (Number(confidence) >= 60) return 'Moderate'
  return 'Needs review'
}
...
function ClassifierAnalysis({ complaint }) {
  ...
  const evidenceLabel = categoryEvidenceLabel(confidence)
  const showAdditionalIssueAnalysis = Boolean(complaint.classification_multi_issue || complaint.classification_ambiguous)
    ...
    <section className="card rounded-xl p-5" aria-labelledby="classification-title">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-xs text-gray-500">Detected complaint type and priority recommendation based on the submitted details.</p>
        </div>
        <div className="flex items-center gap-2">
          <div className="rounded-lg bg-navy-50 px-3 py-2 text-right">
            <p className="text-xs font-black uppercase tracking-wider text-navy-500">Priority score</p>
      ...
      {!hasStoredAnalysis ? (
        <div className="mt-4 rounded-lg border border-dashed border-gray-300 bg-gray-50 p-4 text-center">
          ...
          <p className="mt-1 text-xs text-gray-500">No stored automatic analysis is available for this complaint.</p>
        </div>
      ) : (
        <>
          <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <div className="rounded-lg border border-gray-100 bg-gray-50 p-3"><p className="text-xs font-black uppercase tracking-wider text-gray-500">Detected complaint type</p><p className="mt-1 text-sm font-black text-navy-900">{complaint.classified_category || complaint.complaint_type}</p></div>
            <div className="rounded-lg border border-gray-100 bg-gray-50 p-3"><p className="text-xs font-black uppercase tracking-wider text-gray-500">Category evidence strength</p><div className="mt-1 flex flex-wrap items-baseline gap-2"><p className="text-xl font-black text-navy-900">{confidence ?? '—'}{confidence != null && <span className="text-xs font-normal text-gray-500">%</span>}</p>{evidenceLabel && <span className={`rounded-full px-2 py-0.5 text-xs font-black ${evidenceLabel === 'Strong' ? 'bg-green-50 text-green-700' : evidenceLabel === 'Moderate' ? 'bg-amber-50 text-amber-800' : 'bg-red-50 text-red-700'}`}>{evidenceLabel}</span>}</div></div>
            <div className="rounded-lg border border-gray-100 bg-gray-50 p-3"><p className="text-xs font-black uppercase tracking-wider text-gray-500">Urgency signal</p><span className={`mt-1.5 inline-flex rounded border px-2 py-1 text-xs font-bold ${sentimentStyles[complaint.classification_sentiment] || sentimentStyles.neutral}`}>{urgencyLabel}</span></div>
            <div className="rounded-lg border border-gray-100 bg-gray-50 p-3"><p className="text-xs font-black uppercase tracking-wider text-gray-500">System suggested priority</p><div className="mt-1.5"><PriorityBadge priority={suggestedPriority} /></div></div>
          </div>
          ...
          {showAdditionalIssueAnalysis && (
            <div className="mt-3 rounded-lg border border-gray-200 bg-white p-4" aria-label="Additional issue analysis">
              <div className="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  <p className="text-xs font-black uppercase tracking-wider text-gray-500">Additional issue analysis</p>
                  <p className="mt-1 text-xs text-gray-500">Shown only when another complaint category has meaningful evidence or the leading category needs review.</p>
                </div>
                <span className={`mt-1 inline-flex w-fit rounded-full border px-2.5 py-1 text-xs font-black sm:mt-0 ${complaint.classification_ambiguous ? 'border-amber-200 bg-amber-50 text-amber-900' : 'border-blue-300 bg-blue-100 text-blue-950'}`}>
                  {complaint.classification_ambiguous ? 'Category competition needs review' : 'Multiple issues detected'}
                </span>
              </div>

              <div className="mt-3 grid gap-3 md:grid-cols-2">
                <div className="rounded-lg border border-gray-100 bg-gray-50 p-3">
                  <p className="text-xs font-black uppercase tracking-wider text-gray-500">Other detected issues</p>
                  ...
                  ) : <p className="mt-1 text-sm font-bold text-gray-600">No additional issue met the secondary-issue threshold.</p>}
                </div>
                <div className="rounded-lg border border-gray-100 bg-gray-50 p-3">
                  <p className="text-xs font-black uppercase tracking-wider text-gray-500">Category comparison</p>
                  <p className={`mt-1 text-sm font-black ${complaint.classification_ambiguous ? 'text-amber-800' : 'text-green-700'}`}>
                    {complaint.classification_ambiguous ? 'Competing categories need reviewer confirmation' : 'No strong competing category detected'}
                  </p>
                </div>
              </div>
              ...
              {complaint.classification_multi_issue && (
                <div className="mt-3 rounded-lg border border-blue-200 bg-blue-50 px-3 py-3">
                  <p className="text-xs font-black text-blue-950">More than one supported issue appears in this complaint</p>
                  <p className="mt-0.5 text-xs text-blue-800">Review the detected complaint type and the additional issues before routing or resolving the complaint.</p>
          ...
          <details className="mt-4 rounded-lg border border-gray-200 bg-white">
            <summary className="cursor-pointer px-4 py-3 text-xs font-black text-navy-800">See analysis evidence</summary>
            <div className="grid gap-5 border-t border-gray-100 p-4 lg:grid-cols-2">
              ...
              <div>
                <p className="text-xs font-black uppercase tracking-wider text-gray-500">Scoring explanation</p>
  ...
  return (
    <section className="card rounded-xl p-5 no-print" aria-labelledby="task-actions-title">
      ...
      <div className="mt-4 space-y-3">
        ...
        {isActive && <details className="rounded-lg border border-gray-200 bg-gray-50"><summary className="cursor-pointer px-4 py-3 text-sm font-bold text-navy-800">More options</summary><div className="space-y-2 border-t border-gray-200 p-3"><button onClick={onPlan} className="btn-secondary w-full rounded-lg">Update materials and work notes</button><button onClick={onIssue} className="w-full rounded-lg border border-amber-200 bg-amber-50 px-4 py-2.5 text-sm font-bold text-amber-800">Request help or reassignment</button></div></details>}
      </div>
    </section>
  )
}
...
export default function ComplaintDetailsPage() {
  ...
  return <div className="space-y-5 complaint-receipt">
    ...
    <div className="page-band wave-header page-header"><div className="relative flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="mb-1.5 text-xs font-bold uppercase tracking-[.15em] text-gold-400">Complaint details</p><h1 className="text-white">{complaint.complaint_type}</h1><p className="mt-2 font-mono text-xs font-bold text-navy-300">Reference: {complaint.reference_number}</p></div><div className="flex flex-wrap items-center gap-2">{!isCustomer && complaint.priority && <PriorityBadge priority={complaint.priority} />}<StatusBadge status={complaint.status} /></div></div></div>
    {complaint.service_account_number && <div className="card rounded-xl p-4 text-sm text-gray-700"><span className="font-bold">MRWD service account:</span> <span className="font-mono">{complaint.service_account_number}</span></div>}
    <ComplaintProgress complaint={complaint} />
    ...
    {complaint.status === 'blocked' && <div className="rounded-xl border border-amber-200 bg-amber-50 p-5"><p className="font-display font-bold text-amber-900">{STATUS_LABELS.blocked}</p><p className="mt-1 text-sm text-amber-800">{complaint.reassignment_reason || complaint.unable_reason || 'Maintenance Personnel reported an issue that needs WDLCD review.'}</p></div>}
    ...
    <section className="card rounded-xl p-4 sm:p-5">
      <div className={`grid gap-3 sm:grid-cols-2 lg:grid-cols-3 ${isCustomer ? 'xl:grid-cols-5' : 'xl:grid-cols-6'}`}>
        <div><p className="text-xs font-black uppercase tracking-wider text-gray-500">Status</p><div className="mt-1"><StatusBadge status={complaint.status} /></div></div>
        {!isCustomer && <div><p className="text-xs font-black uppercase tracking-wider text-gray-500">Priority</p><div className="mt-1"><PriorityBadge priority={complaint.priority} /></div></div>}
        <div><p className="text-xs font-black uppercase tracking-wider text-gray-500">Assigned Maintenance Personnel</p><p className="mt-1 truncate text-xs font-bold text-gray-800">{complaint.assigned_name || 'Not assigned'}</p></div>
        <div><p className="text-xs font-black uppercase tracking-wider text-gray-500">Submitted</p><p className="mt-1 text-xs font-semibold text-gray-700">{formatDate(complaint.created_at)}</p></div>
        <div><p className="text-xs font-black uppercase tracking-wider text-gray-500">Sent to WDLCD</p><p className="mt-1 text-xs font-semibold text-gray-700">{formatDate(complaint.forwarded_to_ecmd_at)}</p></div>
        <div><p className="text-xs font-black uppercase tracking-wider text-gray-500">Last updated</p><p className="mt-1 text-xs font-semibold text-gray-700">{formatDate(complaint.updated_at)}</p></div>
      </div>
      {!isCustomer && <div className="mt-4 grid gap-3 border-t border-gray-100 pt-4 sm:grid-cols-2"><div><p className="text-xs font-black uppercase tracking-wider text-gray-500">Current team</p><p className="mt-1 text-xs font-black text-navy-900">{complaint.status === 'pending' || complaint.status === 'rejected' ? 'Commercial Services Department / NSCCCD' : complaint.status === 'merged' ? 'Merged / historical record' : 'ECMD / Water Distribution and Leakage Control Division (WDLCD)'}</p></div><div><p className="text-xs font-black uppercase tracking-wider text-gray-500">Commercial handoff note</p><p className="mt-1 text-xs text-gray-700">{complaint.commercial_handoff_note || 'No handoff note recorded.'}</p></div></div>}
    </section>
    ...
    {complaint.status === 'rejected' && <button onClick={() => setRestoreOpen(true)} className="btn-primary w-full rounded-lg">Undo rejection</button>}
        ...
        {canEcmdOperate && <section className="card rounded-xl p-5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><p className="text-xs font-black uppercase tracking-wider text-navy-600">WDLCD</p><h2 className="mt-1 font-display font-bold text-navy-900">Field dispatch</h2><p className="mt-1 text-xs leading-5 text-gray-500">Assign and manage field work for this complaint.</p></div><div className="self-start"><StatusBadge status={complaint.status} /></div></div>
          <div className="mt-4 space-y-2">
    ...
    <Modal open={priorityOpen} title={canCommercialReview ? 'Change priority score' : 'Change priority'} subtitle={canCommercialReview ? 'The system suggestion stays in the record. Add a reason for every manual change.' : 'WDLCD can change the priority after handoff. Add a reason so the change is clear in the activity log.'} onClose={() => !busy && setPriorityOpen(false)}><form onSubmit={handlePriorityOverride} className="space-y-4">{canCommercialReview ? <><div className="rounded-lg border border-navy-100 bg-navy-50 p-3 text-sm text-navy-800"><span className="font-bold">Suggested priority:</span> {complaint.algorithm_priority_score ?? complaint.priority_score}/100</div><div><label className="block text-xs font-black text-gray-500 uppercase mb-1.5">Priority score (0–100)</label><input type="number" min="0" max="100" step="1" required value={priorityForm.score} onChange={event => setPriorityForm(form => ({ ...form, score: event.target.value }))} className="input-field rounded-lg" /></div></> : <div><label className="block text-xs font-black text-gray-500 uppercase mb-1.5">Priority</label><select required value={priorityForm.priority} onChange={event => setPriorityForm(form => ({ ...form, priority: event.target.value }))} className="input-field rounded-lg"><option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option></select></div>}<div><label className="block text-xs font-black text-gray-500 uppercase mb-1.5">Reason for change *</label><textarea rows={4} required minLength={5} value={priorityForm.reason} onChange={event => setPriorityForm(form => ({ ...form, reason: event.target.value }))} className="input-field rounded-lg resize-none" placeholder="Explain why this complaint needs a different priority." /></div><div className="grid gap-2 sm:grid-cols-2">{canCommercialReview && complaint.priority_is_overridden && <button type="button" onClick={handlePriorityReset} disabled={busy || priorityForm.reason.trim().length < 5} className="btn-secondary rounded-lg disabled:opacity-50 sm:col-span-2">Use suggested priority</button>}<button type="button" onClick={() => setPriorityOpen(false)} className="btn-secondary rounded-lg">Cancel</button><button disabled={busy || priorityForm.reason.trim().length < 5} className="btn-primary rounded-lg disabled:opacity-50">Save priority change</button></div></form></Modal>

    <Modal open={editOpen} title="Edit complaint" onClose={() => !busy && setEditOpen(false)}>
      <form onSubmit={handleEdit} className="space-y-4">
        <div><label className="block text-xs font-black text-gray-500 uppercase mb-1.5">Type</label><select name="complaintdetailspage-complaint-type-5" aria-label="Complaint type" value={editForm.complaint_type} onChange={event => setEditForm(form => ({...form, complaint_type:event.target.value}))} className="input-field rounded-lg">{COMPLAINT_TYPES.map(type => <option key={type}>{type}</option>)}</select></div>
        <div>
          <label htmlFor="edit-complaint-description" className="block text-xs font-black text-gray-500 uppercase mb-1.5">Description</label>
          <textarea id="edit-complaint-description" name="complaintdetailspage-description-6" aria-label="Description" rows={5} required minLength={COMPLAINT_DESCRIPTION_MIN_LENGTH} maxLength={COMPLAINT_DESCRIPTION_MAX_LENGTH} value={editForm.description} onChange={event => setEditForm(form => ({...form, description:event.target.value}))} className="input-field rounded-lg resize-none" />
          <p className="mt-1.5 flex items-start justify-between gap-3 text-xs text-gray-500"><span>Use at least {COMPLAINT_DESCRIPTION_MIN_LENGTH} characters and include the relevant details.</span><span className="shrink-0 font-bold">{editForm.description.length.toLocaleString()}/{COMPLAINT_DESCRIPTION_MAX_LENGTH.toLocaleString()}</span></p>
        </div>
        <div><label className="block text-xs font-black text-gray-500 uppercase mb-1.5">Address</label><input name="complaintdetailspage-address-7" aria-label="Address" required minLength={COMPLAINT_ADDRESS_MIN_LENGTH} value={editForm.address} onChange={event => setEditForm(form => ({...form, address:event.target.value}))} className="input-field rounded-lg" /></div>
        <div className="flex justify-end gap-2"><button type="button" onClick={() => setEditOpen(false)} className="btn-secondary rounded-lg">Cancel</button><button disabled={busy} className="btn-primary rounded-lg">Save changes</button></div>
      </form>
    </Modal>
    ...
    <Modal open={forwardOpen} title="Send complaint to WDLCD" subtitle="NSCCCD has reviewed this complaint. Sending it places the complaint in the WDLCD dispatch queue under ECMD." onClose={() => !busy && setForwardOpen(false)}><div className="space-y-4"><div><label htmlFor="wdlcd-handoff-note" className="mb-1.5 block text-xs font-black uppercase text-gray-500">Handoff note</label><textarea id="wdlcd-handoff-note" name="wdlcd-handoff-note" data-dialog-initial-focus rows={4} value={forwardNote} onChange={e => setForwardNote(e.target.value)} className="input-field resize-none rounded-lg" placeholder="Optional note for WDLCD"/></div><div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><button type="button" onClick={() => setForwardOpen(false)} className="btn-secondary rounded-lg">Cancel</button><button type="button" onClick={handleForward} disabled={busy} className="btn-primary rounded-lg disabled:opacity-50">{busy ? 'Sending…' : 'Send to WDLCD'}</button></div></div></Modal>
