...
export default function ReportsPage() {
  ...
  return (
    <div className="space-y-5 report-print-area">
      <div className="page-band wave-header rounded-2xl px-6 py-6 no-print"><div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4"><div><p className="text-gold-400 text-[11px] font-bold uppercase tracking-widest">Admin · Analytics</p><h1 className="font-display font-black text-white text-2xl sm:text-3xl mt-1">Reports & Exports</h1><p className="text-navy-300 text-sm mt-1">Complaint volume, resolution performance, workload, and satisfaction.</p></div><div className="flex gap-2"><button onClick={exportCsv} className="px-4 py-2 rounded-lg bg-white text-navy-800 text-xs font-black">Export CSV</button><button onClick={() => window.print()} className="px-4 py-2 rounded-lg border border-white/40 text-white text-xs font-black">Print / Save PDF</button></div></div></div>
      ...
      <div className="card rounded-xl p-5"><div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4"><div><h2 className="font-display font-bold text-navy-900">Operational Measures</h2><p className="text-xs text-gray-400 mt-1">Calculated from completed complaints and customer feedback.</p></div><div className="text-sm text-gray-600"><b>{summary.average_resolution_hours ?? '—'}</b> average resolution hours · <b>{summary.feedback_count ?? 0}</b> feedback responses</div></div>
        <div className="overflow-x-auto"><table className="w-full min-w-[680px] text-sm"><thead><tr className="bg-gray-50 border-b-2 border-gray-200 text-left">{['Technician', 'Availability', 'Active', 'Completed', 'Total', 'Completion Rate'].map(item => <th key={item} className="px-4 py-3 text-xs font-black text-gray-400 uppercase">{item}</th>)}</tr></thead><tbody className="divide-y divide-gray-100">{(data?.technician_workload || []).map(person => <tr key={person.id}><td className="px-4 py-3 font-bold text-gray-900">{person.name}{!person.is_active && <span className="ml-2 text-[10px] text-red-600">INACTIVE</span>}</td><td className="px-4 py-3 capitalize text-gray-600">{titleCase(person.availability_status)}</td><td className="px-4 py-3">{person.active}</td><td className="px-4 py-3">{person.completed}</td><td className="px-4 py-3">{person.total}</td><td className="px-4 py-3 font-bold text-navy-800">{person.completion_rate}%</td></tr>)}</tbody></table></div>
      </div>
    </div>
  )
}
