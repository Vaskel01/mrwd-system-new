    ...
    <div className="space-y-5">
      ...
      {notifications.length === 0 ? (
        <div className="card rounded-xl p-14 text-center">
          ...
          <h2 className="font-display font-bold text-navy-900">You are all caught up</h2>
          <p className="text-sm text-gray-400 mt-2">New updates will appear here.</p>
        </div>
      ) : (
        <>
              ...
              <article key={item.id} className={`p-4 sm:p-5 flex gap-3 transition-colors ${item.read_at ? 'bg-white' : 'bg-brand-50/50'}`}>
                <button type="button" onClick={() => openNotification(item)} className="min-w-0 flex flex-1 gap-4 text-left hover:opacity-90">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {item.related_complaint_id ? <p className="text-xs font-bold text-brand-700 mt-2">Open related complaint →</p> : null}
                  </div>
                  {!item.read_at ? <span className="w-2 h-2 rounded-full bg-brand-500 mt-2 shrink-0" aria-label="Unread" /> : null}
                </button>
                <button type="button" onClick={() => dismissNotification(item)} className="self-start rounded-lg p-2 text-gray-400 hover:bg-red-50 hover:text-red-600" aria-label={`Dismiss notification: ${item.title}`}>
