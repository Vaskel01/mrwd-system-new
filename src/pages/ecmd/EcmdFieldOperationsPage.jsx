...
export default function EcmdFieldOperationsPage() {
  ...
  const recurringLocations = useMemo(() => {
    ...
    const eventTime = item => new Date(item.created_at || item.submitted_at || item.updated_at || 0).getTime() || 0
    return Object.values(groups)
      .filter(group => group.length >= 2)
      .map(group => [...group].sort((a, b) => eventTime(b) - eventTime(a)))
      .sort((a, b) => b.length - a.length || eventTime(b[0]) - eventTime(a[0]))
      .slice(0, 8)
  }, [complaints])
  ...
  return <div className="space-y-5">
    ...
    <section className="grid gap-5 xl:grid-cols-2">
      ...
      <div className="card rounded-xl p-5"><h2 className="font-display font-black text-navy-900">Recurring locations</h2><p className="mt-1 text-xs text-gray-500">Repeated complaint addresses based on normalized written address text. These are historical repeats, not distance-based map clusters.</p><div className="mt-4 space-y-2">{recurringLocations.length ? recurringLocations.map(group => <button key={group[0].address} onClick={() => navigate(`/complaints/${group[0].id}`)} className="w-full rounded-lg border border-gray-100 bg-gray-50 p-3 text-left hover:bg-gray-100"><div className="flex items-start justify-between gap-3"><p className="text-sm font-bold text-gray-900">{group[0].address}</p><span className="rounded-full bg-amber-100 px-2 py-0.5 text-xs font-black text-amber-800">{group.length} records</span></div><p className="mt-1 text-xs text-gray-500">Latest complaint: {group[0].reference_number}</p></button>) : <p className="py-6 text-center text-sm text-gray-500">No recurring complaint locations found.</p>}</div></div>
    </section>
