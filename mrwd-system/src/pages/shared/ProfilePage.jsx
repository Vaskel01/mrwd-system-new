...
export default function ProfilePage() {
  ...
  return (
    <div className="space-y-5">
      <div className="page-band wave-header rounded-2xl px-6 py-6"><p className="text-gold-400 text-[11px] font-bold uppercase tracking-widest">Account</p><h1 className="font-display font-black text-white text-2xl sm:text-3xl mt-1">My profile</h1><p className="text-navy-300 text-sm mt-1">Update your personal details, availability, notifications, password, and account security.</p></div>
      ...
      {effectiveRole !== 'customer' && <section className="card max-w-2xl rounded-xl p-5 sm:p-6 space-y-4">
        ...
        <div className="rounded-xl border border-gray-200 p-4">
          ...
          {(mfaStatus?.factors?.totp || []).filter(item => item.status === 'verified').map(item => <div key={item.id} className="mt-3 flex flex-wrap items-center justify-between gap-3 rounded-lg bg-gray-50 p-3"><div><p className="text-xs font-black text-gray-800">{item.friendly_name || 'MRWD Authenticator'}</p><p className="text-[11px] text-gray-500">Authenticator connected</p></div><button type="button" disabled={mfaBusy} onClick={() => removeMfa(item.id)} className="btn-secondary rounded-lg text-xs">Remove</button></div>)}
