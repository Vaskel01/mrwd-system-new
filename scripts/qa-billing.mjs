...
try {
  ...
  for (const [width, theme] of [[1440, 'light'], [768, 'dark'], [390, 'light'], [390, 'dark']]) {
    ...
    assert.equal(await page.getByRole('table').isVisible(), width >= 1024, 'Desktop table visibility')
    const button = page.getByRole('button', { name: 'View statement September 2026 for account QA-001', exact: true }).filter({ visible: true })
    await button.click()
