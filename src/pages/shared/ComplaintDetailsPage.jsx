...
function MaintenanceTaskActions({
  ...
  return (
    <section className="card rounded-xl p-5 no-print" aria-labelledby="task-actions-title">
      ...
      <p className="mt-1 text-xs text-gray-500">Update field progress and record what was done so ECMD can review the work.</p>
      <div className="mt-4 space-y-3">
        {nextTaskStatus ? (
          ...
          <div className="rounded-lg border border-violet-200 bg-violet-50 px-4 py-3 text-sm font-bold text-violet-800">Field work is complete and waiting for ECMD verification.</div>
...
export default function ComplaintDetailsPage() {
  ...
  return <div className="space-y-5 complaint-receipt">
    ...
    {complaint.status === 'merged' && <div className="rounded-xl border border-violet-200 bg-violet-50 p-5"><p className="font-display font-bold text-violet-900">Merged with another complaint</p><p className="mt-1 text-sm text-violet-800">This duplicate remains in the history but no longer has separate field work.</p><p className="mt-2 text-xs text-violet-700">Reason: {complaint.merge_reason || 'Duplicate complaint.'}</p>{complaint.merged_into_id && <button onClick={() => navigate(`/complaints/${complaint.merged_into_id}`)} className="btn-secondary mt-3 rounded-lg text-xs">Open main complaint</button>}</div>}
    ...
    <section className="card rounded-xl p-4 sm:p-5">
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
        ...
        <div><p className="text-[11px] font-black uppercase tracking-wider text-gray-500">Forwarded to ECMD</p><p className="mt-1 text-xs font-semibold text-gray-700">{formatDate(complaint.forwarded_to_ecmd_at)}</p></div>
    ...
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_320px] xl:grid-cols-[minmax(0,1fr)_340px]">
      ...
      <aside className="min-w-0 space-y-4 lg:self-start lg:sticky lg:top-5 no-print">
        {canCommercialReview && <section className="card rounded-xl p-5">
          <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-wider text-brand-600">Commercial Services</p><h2 className="mt-1 font-display font-bold text-navy-900">Complaint review</h2><p className="mt-1 text-xs text-gray-500">Check the complaint details and send it to ECMD when it is ready for field work.</p></div><StatusBadge status={complaint.status} /></div>
          <div className="mt-4 space-y-2">
            {complaint.status === 'pending' && <button onClick={() => setForwardOpen(true)} className="w-full rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-black text-white hover:bg-blue-700">Send to ECMD</button>}
    {complaint.status === 'rejected' && <button onClick={() => setRestoreOpen(true)} className="btn-primary w-full rounded-lg">Undo rejection</button>}
        ...
        {canEcmdOperate && <section className="card rounded-xl p-5">
          <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-wider text-violet-600">ECMD</p><h2 className="mt-1 font-display font-bold text-navy-900">Dispatch and verification</h2><p className="mt-1 text-xs text-gray-500">Assign field work and verify completed work before resolving the complaint.</p></div><StatusBadge status={complaint.status} /></div>
          <div className="mt-4 space-y-2">
            {complaint.status === 'awaiting_verification' && <button onClick={() => { setVerifyForm({ resolution_code: 'resolved', resolution_notes: '', return_to_field: false }); setVerifyOpen(true) }} className="w-full rounded-lg bg-violet-600 px-4 py-2.5 text-sm font-black text-white hover:bg-violet-700">Verify completed work</button>}
            ...
            {!['forwarded','assigned','en_route','in_progress','blocked','awaiting_verification'].includes(complaint.status) && <p className="rounded-lg bg-gray-50 p-3 text-xs text-gray-500">This complaint is not ready for ECMD assignment right now.</p>}
          </div>
    ...
    <Modal open={completeOpen} title="Mark field work complete" subtitle="Describe the work performed. ECMD will verify the resolution before the complaint is closed." onClose={() => !busy && setCompleteOpen(false)}><form onSubmit={handleComplete} className="space-y-4">{templates.length > 0 && <div><p className="mb-2 text-xs font-black uppercase text-gray-500">Note templates</p><div className="flex flex-wrap gap-2">{templates.map(t => <button key={t.id} type="button" onClick={() => setCompletion(v => ({...v, completion_notes:t.content}))} className="rounded-lg border border-gray-200 bg-gray-50 px-2.5 py-1.5 text-xs font-bold text-gray-700 hover:border-navy-300">{t.label}</button>)}</div></div>}<div><label className="block text-xs font-black text-gray-500 uppercase mb-1.5">Work completed *</label><textarea rows={5} required minLength={5} value={completion.completion_notes} onChange={event => setCompletion(value => ({...value, completion_notes:event.target.value}))} className="input-field rounded-lg resize-none" placeholder="Describe what was inspected, repaired, replaced, or restored." /></div><div><label className="block text-xs font-black text-gray-500 uppercase mb-1.5">Materials used</label><textarea rows={3} value={completion.materials_used} onChange={event => setCompletion(value => ({...value, materials_used:event.target.value}))} className="input-field rounded-lg resize-none" /></div><div className="flex justify-end gap-2"><button type="button" onClick={() => setCompleteOpen(false)} className="btn-secondary rounded-lg">Cancel</button><button disabled={busy || completion.completion_notes.trim().length < 5} className="rounded-lg bg-green-600 px-5 py-2.5 text-sm font-bold text-white disabled:opacity-50">{busy ? 'Submitting…' : 'Send for ECMD verification'}</button></div></form></Modal>

    <Modal open={forwardOpen} title="Send complaint to ECMD" subtitle="Commercial Services has reviewed this complaint. Forwarding places it in the ECMD dispatch queue." onClose={() => !busy && setForwardOpen(false)}><div className="space-y-4"><div><label className="block text-xs font-black uppercase text-gray-500 mb-1.5">Handoff note</label><textarea rows={4} value={forwardNote} onChange={e => setForwardNote(e.target.value)} className="input-field resize-none rounded-lg" placeholder="Optional note for ECMD"/></div><div className="flex justify-end gap-2"><button onClick={() => setForwardOpen(false)} className="btn-secondary rounded-lg">Cancel</button><button onClick={handleForward} disabled={busy} className="rounded-lg bg-blue-600 px-5 py-2.5 text-sm font-bold text-white">{busy ? 'Forwarding…' : 'Send to ECMD'}</button></div></div></Modal>
