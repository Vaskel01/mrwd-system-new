...
export default function MetricCard({ label, value, detail, icon, onClick, accent }) {
  const Component = onClick ? 'button' : 'div'
  return (
    <Component
      type={onClick ? 'button' : undefined}
      onClick={onClick}
      className={`card w-full rounded-xl p-5 text-left ${onClick ? 'transition hover:-translate-y-0.5 hover:border-navy-300 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-navy-300' : ''}`}
    >
